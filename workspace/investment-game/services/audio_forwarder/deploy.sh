#!/bin/bash
set -e

if [[ -z "${PEPPER_PASS:-}" ]]; then
    echo "Error: PEPPER_PASS is not set. Please export PEPPER_PASS first."
    exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

PEPPER_USER="nao"
PEPPER_HOST="pepper.local"
PEPPER_PATH="/home/nao"

LOCAL_SCRIPT="$SCRIPT_DIR/audio_forwarder.py"

echo "Copying $LOCAL_SCRIPT to $PEPPER_USER@$PEPPER_HOST:$PEPPER_PATH ..."
sshpass -p "$PEPPER_PASS" scp "$LOCAL_SCRIPT" "$PEPPER_USER@$PEPPER_HOST:$PEPPER_PATH"

echo "Starting audio forwarder on Pepper..."
sshpass -p "$PEPPER_PASS" ssh "$PEPPER_USER@$PEPPER_HOST" << EOF
pkill -f audio_forwarder.py 2>/dev/null

nohup python2.7 $PEPPER_PATH/audio_forwarder.py > $PEPPER_PATH/audio_forwarder.log 2>&1 &
EOF
